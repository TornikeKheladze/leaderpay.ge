...
function renderScoreHud() {
    if (isCasualGame()) {
        ...
        scoreNode.innerText = `ქულა: ${state.game.score}`;
        scoreNode.style.display = 'block';
        cubeCountNode.style.opacity = 0.65;
    }
    cubeCountNode.innerText = `დამსხვრეული კუბები: ${state.game.cubeCount}`;
}
...
function renderMenus() {
    ...
    switch (state.menus.active) {
        ...
        case MENU_SCORE:
            ...
            if (isNewHighScore()) {
                highScoreLblNode.textContent = 'ახალი რეკორდი!';
            } else {
                highScoreLblNode.textContent = `რეკორდი: ${formatNumber(getHighScore())}`;
            }
